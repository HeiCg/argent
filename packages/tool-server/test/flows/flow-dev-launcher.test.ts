...
// on the run's own port that had long stopped answering — which is exactly the
// shape that makes "just tap the row with the right port" wrong.
...
describe("picking the row for the run's own bundler", () => {
  ...
  it("never falls back to a remembered row, even one carrying the right port", () => {
    // Only the history holds 8083 — and history rows are stale by nature (the
    // fixture's own 192.168.92.72:8081 is a dead address on the live port).
    // Reporting beats opening a server that may not answer.
    const tree = launcherTree();
    expect(pickDevServerRow(tree, emulator, 8083, historyY(tree))).toBeNull();
  });
