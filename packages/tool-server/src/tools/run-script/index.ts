...
The script body is plain JavaScript (NOT TypeScript) executed locally in THIS tool-server process inside a node:vm sandbox, with only two injected globals: \`ui\` (an async device facade — describe / find / exists / visible / tap / fill / pressKey / button / swipe / scrollUntilVisible / await / awaitIdle / launchApp / openUrl / sleep) and a captured \`console\`. There is no require, import, process, fs, or network. \`await\` every ui.* call. The full \`ui\` typings and worked examples live in the \`argent-device-interact\` skill.
