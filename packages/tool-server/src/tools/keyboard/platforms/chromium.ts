...
async function runChromium(api: ChromiumCdpApi, params: KeyboardParams): Promise<KeyboardResult> {
  ...
  try {
    ...
    // The tool rejects a request carrying both `text` and `key` (see
    // ../index.ts), so this block and the text loop above are mutually
    // exclusive. Only `clear` combines with either, and it runs before both.
    if (named) {
      await api.dispatchKeyEvent({
        type: "keyDown",
        key: named.key,
