...
- clear: empties the focused field before typing. Typing alone APPENDS — against a field that already holds a value (a remembered login, a restored draft, a re-run step) the old text stays and the new text lands after it. Use \`{ clear: true, text: "…" }\` to replace a value, \`{ clear: true }\` alone to just empty it. iOS, Android and Chromium; rejected on Vega and TV targets. Focus a text field first — on Chromium a clear with nothing editable focused is refused outright, and on iOS/Android it is dispatched blind. \`cleared: true\` means the field was observed empty only on Chromium; elsewhere it means the clear was dispatched, so assert the value when it matters. On Android levels older than \`input keycombination\` the clear deletes backwards from end-of-LINE, so a multi-line field keeps what sits below the caret; a field over 200 characters is refused rather than partly deleted, and a password field (whose length is unreadable) gets a fixed 160 backspaces.
