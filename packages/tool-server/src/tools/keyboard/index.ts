  ...
  clear: z
    ...
    .describe(
      "Set to true to empty the focused text field. Tap the field first — the clear goes wherever keyboard focus is, and " +
        ...
        "Chromium instead selects the focused editable, deletes the selection (no length limit) and then DOES read the " +
        "field back, so `cleared` there means it was seen empty and the result also carries `clearVerified: true` — the " +
        "one structural way to tell the two meanings of `cleared` apart; `keys` is 0, because it dispatches no key events. It " +
