...
A rejected request almost never changes the field. An unsupported key name, un-typeable text, and a \`clear\` on a platform that cannot do it are all decided before anything is sent, on every backend; the Android over-length refusal is decided from a READ of the field, so a support probe and a screen dump go to the device first — neither of which changes it. The one exception is Android's accessibility replace: if a widget ACCEPTS it and the field then reads back holding something else, the injected fallback runs and can still refuse on length — so that 400 follows a write. The failure names it. Either way a 400 leaves the field exactly as it was, neither emptied nor half-typed. What is NOT rolled back is a transport failure partway through: the characters already sent stay in the field. On a retry after one of those, read the field's actual contents — do not assume it is unchanged.
