  ...
  clear: z
    ...
    .describe(
      "Empty the focused text field before typing. Use this whenever a field may already hold a value — typing alone does NOT replace it: the old text survives and the new text goes in at the caret, which lands after the old value or splices into the middle of it depending on where focus left the caret. " +
        ...
        "What each backend can leave behind: Chromium and Android read the field back after the select-all and delete whatever survived it, so a widget that swallows the chord is corrected rather than trusted — on iOS, which cannot read the field at all, that same widget leaves the delete acting as a plain backspace AT THE CARET, so the field ends up one character shorter rather than unchanged and a combined `text` is appended at the end (where focusing jumped). " +
        "On Android the read-back needs the screen's view hierarchy; where that cannot be captured the clear stays best-effort like iOS. On levels older than `input keycombination` the clear deletes backwards from end-of-LINE, so a multi-line field keeps what sits below the caret, and a field over 150 characters is refused rather than partly deleted; a length that cannot be read at all (a password field, or a screen the device would not capture) falls back to a fixed 158 backspaces — more than every length that path accepts, so a field only keeps its head past 158 characters."
    ),
  delayMs: z
    .number()
