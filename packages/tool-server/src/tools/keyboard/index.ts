...
A rejected request never reaches the device. An unsupported key name, un-typeable text, and a \`clear\` on a platform that cannot do it are all checked before anything is sent, on every backend — so a 400 leaves the field exactly as it was, neither emptied nor half-typed. What is NOT rolled back is a transport failure partway through: the characters already sent stay in the field. On a retry after one of those, read the field's actual contents — do not assume it is unchanged.
