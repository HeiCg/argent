...
const zodSchema = z.object({
  ...
  bootTimeoutMs: z
    ...
    .optional()
    .describe(
      "Android/Vega: overall budget for the boot sequence. Default 480000 (8 min) on Android, 120000 (2 min) on Vega. Clamped to [30s, 15min]. Ignored on iOS and on Electron, which waits its own fixed 30s for CDP."
    ),
  force: z
    .boolean()
    .optional()
