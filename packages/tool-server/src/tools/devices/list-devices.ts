...
iOS entries likewise carry a 'kind' ('simulator', or 'device' for a connected physical iPhone). Physical iOS devices require the 'physical-ios-devices' flag (\`argent enable physical-ios-devices\`) and a device with Developer Mode enabled that has been connected to Xcode/devicectl at least once (so the CoreDevice Developer Disk Image is mounted); they are driven root-free over USB by the simulator-server and support screenshot, gesture-tap, gesture-swipe, button, and launch-app.
