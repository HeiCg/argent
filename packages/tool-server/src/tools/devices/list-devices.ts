...
Use at the start of a session to pick a target id ('udid' for iOS entries, including the 'remote:'-prefixed one an ios-remote entry carries; 'serial' for Android/Vega entries, 'id' for Chromium) to pass to interaction tools, and to see which targets are already running. A remote simulator has to be named with --device: flow auto-detection never selects one.
Returns { devices, avds } where each device carries a 'platform' discriminator ('ios', 'ios-remote', 'android', 'chromium', or 'vega'); 'avds' lists Android AVDs bootable via boot-device. A Vega VVD is listed under 'devices' whether running or stopped (state 'running'/'stopped'); start a stopped one with boot-device using its 'vvdImage'.
...
TV targets are tagged with runtimeKind 'tv' (Apple TV simulators, local or remote; Android TV / leanback devices) — these are focus-driven, not touch-driven: use \`describe\` to read focus, \`tv-remote\` for remote presses (up/down/left/right/select/back/menu/home), and \`keyboard\` to type, rather than the coordinate/gesture tools. The field is absent only on an Android device whose feature list could not be read, and such a device is excluded from a runtimeKind-constrained flow auto-detection.
