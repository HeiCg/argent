...
iOS entries likewise carry a 'kind' ('simulator', or 'device' for a connected physical iPhone). Physical iOS devices require the 'physical-ios-devices' flag (\`argent enable physical-ios-devices\`) and a device with Developer Mode enabled that has been connected to Xcode/devicectl at least once (so the CoreDevice Developer Disk Image is mounted); they are driven root-free over USB by the simulator-server. Capture, single-touch input, hardware buttons, typing, accessibility reads and app lifecycle all work on hardware; multi-touch and anything that has to run code inside the target app do not, and reject a 'device' udid with a clear "not supported" error instead of failing on the device.
