...
iOS entries likewise carry a 'kind' ('simulator', or 'device' for a connected physical iPhone). Physical iOS devices require the 'physical-ios-devices' flag (\`argent enable physical-ios-devices\`) and a device with Developer Mode enabled that has been connected to Xcode/devicectl at least once (so the CoreDevice Developer Disk Image is mounted); they are driven root-free over USB by the simulator-server. Capture, touch input up to two simultaneous contacts (pinch included; a two-finger rotation is not recognized on hardware), hardware buttons, typing, accessibility reads and app lifecycle all work on hardware; every tool whose backend is simulator-only - the ones that inject argent's devtools into the app, and the ones that drive simctl - does not, and rejects a 'device' udid with a clear "not supported" error instead of failing on the device; each tool's own capability declaration is what decides, and the README's physical-iOS section lists the current set.
