...
Returns { message, toolResult, stepCount, recorded, savedTo } on success. If it fails an error is returned and nothing is recorded. One success also records nothing: a tap that opens a server row on the Android expo-dev-client chooser, straight after a recorded \`launch\` — that launch opens the row itself at replay. The tool call still ran on the device; \`recorded\` opens with "(not recorded)" and \`message\` says why.
