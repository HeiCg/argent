...
Returns { message, toolResult, stepCount, recorded, savedTo } on success — \`message\` is \`Step added to "<name>" flow\` plus any warning about what was recorded (read it; a warning never means the step was skipped). If it fails an error is returned and nothing is recorded; whether the tool call itself RAN depends on where it failed. The checks that precede the call — the recording must be open, \`args\` must parse, and a recorded \`flow-execute\` must name a sibling flow rather than a \`flow_path\` — reject without running anything. Every later failure means the call ran and nothing it did was undone, so read the error before you retry: repeating such a call repeats the action.
