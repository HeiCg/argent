...
Returns { message, toolResult, stepCount, recorded, savedTo } on success — \`message\` is \`Step added to "<name>" flow\` plus any warning about what was recorded (read it; a warning never means the step was skipped). If it fails an error is returned and nothing is recorded. Two calls answer with what to do instead of running anything: a \`command\` naming a recording tool, and one naming a flow-file directive rather than a tool. Usually that answer is the call to make (the tool that records that directive, or the recording tool called directly), but \`wait\` and \`long-press\` have no recording tool, so those name no call: \`wait\` points at \`await-ui-element\` as the readiness check to record in its place, and \`long-press\` says to add the step by hand during polish. Either way nothing runs at the device, both omit \`recorded\`, and the take is left untouched — read \`recorded\`, not the status, to know whether a step was appended.
