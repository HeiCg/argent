...
Returns { message, toolResult, stepCount, recorded, savedTo } on success. If it fails an error is returned and nothing is recorded — but the tool call itself has already run by then, so read the error before you retry: repeating the call repeats the action.
