...
export interface StepReport {
   ...
   * `tap`/`long-press`, centre-anchored `pinch`/`rotate`) that a tree-source
   * outage left unsettled: it is dispatched regardless, and the warning is the
   * only thing separating it from one that waited.
   */
  warning?: string;
  /**
