...
What the script prints is discarded, here and at replay: \`reason\` is the whole account of a failure, so a script whose only report of itself is a console line has to \`throw\` that line instead. On a PASS \`reason\` carries any note about how the step really ran, such as a time limit clamped to this host's maximum — read it, because the step is recorded with the timeout you asked for. \`outputJson\` is the document the script returned; no flow step can reference it yet.
