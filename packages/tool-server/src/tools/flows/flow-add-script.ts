...
\`log\` is the script's stdout and stderr, capped at 64 KiB. A log the cap cut carries no marker in the text, so read \`logTruncated\` rather than trusting what you see. \`outputJson\` is the document the script returned; no flow step can reference it yet.
