  ...
  .superRefine((params, ctx) => {
    ...
    if (named === (params.flow_path !== undefined)) {
      ctx.addIssue({
        ...
        // flow under a key zod stripped, so that one needs the alias spelled out.
        message: named
...
const fileInputs: FileInputSpec[] = [
  ...
  // Two specs, one target, matching flow-execute: the `name` spec is LAST so it
  // wins the client's last-write-wins merge when both are sent.
  {
    target: "flow_file",
...
Address the flow exactly as you will address it in flow-execute: name or flow_path, one and only one; supplying both or neither is rejected. The name goes in \`name\`, which resolves <project_root>/.argent/flows/<name>.yaml; \`flow_name\` is an accepted alias, and \`name\` wins if both are sent.`,
  ...
  async execute(_services, params, ctx?: ToolContext) {
    ...
    // these params. Same alias fold too: only a name call resolves one, since a
    // flow_path call names no flow.
    const named =
