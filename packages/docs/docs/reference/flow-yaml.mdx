...
`.sh` support is newer than `.mjs` support. A flow that names a `.sh` does not parse on an Argent that predates it. The error names the extension. Run `argent --version` on the host that reports the error.
...
A `.sh` file is a trusted script, the same as an `.mjs` file. Argent does not sandbox it. It has the reach of the account that runs the tool-server.
...
A failed step that writes no reason shows only the exit code. Two exit codes have a meaning of bash: 127 means "command not found", and 126 means "found, but Argent could not run it". Code 126 has two causes: bash cannot read the file, or a command in the file is not executable.
...
Argent runs each candidate once and asks it for its `$BASH_VERSION`. bash sets that name and no other shell does. A candidate that answers without one is not a bash, and Argent does not run a `.sh` step under it: the step would report the document Argent seeded rather than the one the script writes. A candidate on the PATH that fails this check is passed over. A `scripts.bash` that fails it errors the step, and the message names the file the value came from.
...
macOS ships bash 3.2 at `/bin/bash`. Homebrew installs bash 5 earlier on the PATH. Associative arrays, `${var,,}` and `mapfile` need bash 4 or later. Set `scripts.bash` to pin one version. The report names the interpreter path when the script exits with a code, when a signal kills bash, and when bash cannot start. A time limit, a bad document and a missing file name no path.
...
- **A `TERM` trap gets 1.5 seconds.** Argent then sends `KILL` to the group. bash also holds a trap until the foreground command returns
- **Do not write `kill 0`.** `trap 'kill 0' EXIT` is a usual way to reap background jobs, but `kill 0` signals the whole process group of the step. It kills bash itself, so the step fails and the document is lost. Argent stops a background job for you. Signal the pid of the job if you must do it yourself

Argent writes `$ARGENT_OUTPUT` and `$ARGENT_REASON` into a private directory of the step. The directory is readable by your user only. Argent removes the directory when the step ends. A tool-server that dies during the step leaves the directory behind; the next bash step on a new tool-server then removes it. The report never carries the document.
