...
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Action (`tap`, `long-press`, `type`, `scroll-to`, `pinch`, `rotate`) | The most specific visible match: a whole-text or exact-id match beats a substring, a literal spelling beats one only folding makes equal, then the smallest frame, then reading order |
...
A value that holds only invisible characters folds to nothing. Such a value matches nothing, not every element. Use `any: true` with a relational scope for "any element in there".
