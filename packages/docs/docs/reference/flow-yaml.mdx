...
| `timeout` | no       | The limit of the step in milliseconds. The default is 30000                                                                |

Argent runs the script with the project root as the working directory. The script does not inherit your environment. Argent does not read a project `.env` file. Argent passes only `PATH`, `HOME`, the proxy and TLS names, and the Node, Android and Java toolchain names. There is no `env` key. Let the script read a secret or a URL from a file.
...
`scripts.maxTimeoutMs` caps the `timeout` of each step, and `scripts.heapLimitMb` sets the heap of the process. Argent reads both keys from the global configuration file only. See [Available keys](/docs/reference/configuration#available-keys).
