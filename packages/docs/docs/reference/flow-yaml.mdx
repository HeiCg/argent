...
| Vega, TV     | Use `tool: tv-remote` and `tool: keyboard`. `clear` is unsupported on Vega, Apple TV and Android TV. On Vega, `keyboard` also takes `key`; on the two TV targets it takes `text` only. The touch directives are unsupported on all three |
