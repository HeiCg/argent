...
Argent skips leading `echo` and `script` steps when it classifies a flow. A flow that opens with an `echo` or `script` step and then a `launch:` step is still end-to-end. Put the named start state of an end-to-end flow in a leading `echo` step.
...
A `script` step runs a local `.mjs` file in a new Node process.
...
| `timeout` | no       | The time limit of the step in milliseconds. The default is 30000                                                                   |

The working directory is the project root. Argent passes a fixed list of environment variables: the standard shell names, the proxy and TLS names, and the Node, npm, Android and Java toolchain names. A name outside the list does not reach the script, for example `DATABASE_URL`. Argent does not read a project `.env` file. Let the script read a secret or a URL from a file.

The script can set the `output` global. No step reads the value yet, and Argent refuses a flow file that writes `{{output:...}}` into a step: the run stops before step 1. Argent looks for the reference in each field that a later release resolves it in:
...
Argent prints the standard output and the standard error below the step line, on a pass and on a failure. The limit is 64 KiB for one step and 256 KiB for the run. The report marks a cut on its own line. **Argent does not redact this log.** Do not print a credential from a script.
...
On Chromium, Argent boots the app before step 1. A script above the `launch` step therefore runs while the app is up.
