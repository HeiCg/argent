...
Two groups in the list carry access to a credential. `SSH_AUTH_SOCK` reaches your SSH agent. An `npm_config_` name can hold a registry token. Run a script only when you trust it.
