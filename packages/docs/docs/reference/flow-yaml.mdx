...
Argent discards what the script writes to `stdout` and to `stderr`. A recording discards it, and a replay discards it. The report of a step holds no console output. The result of a script is the verdict, plus the document that the script returns. A script that must report a detail throws an error with that detail. The message of the error becomes the reason of the step.
...
The file name without `.yaml` names the report and the artifacts. It contains only letters, numbers, `_` and `-`. A directory run prints only the steps that need attention: each failure and each warning. It also prints a summary. An invalid file fails alone and the batch continues. An infrastructure error stops the batch and counts the remaining flows as skipped.
