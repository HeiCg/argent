...
Write the output document to a sibling file, then move the file into place. A redirection to `$ARGENT_OUTPUT` empties the file before the command that writes it runs. Argent refuses an empty file. Argent also refuses a file that is missing, a document that is not a JSON object, and a document above 1 MiB.
