...
| `$ARGENT_OUTPUT` | A file that holds the output document as JSON. Argent reads it after an exit code of 0              |
| `$ARGENT_REASON` | A file for the failure text. Argent reads it after a different exit code, and adds it to the report |
...
A failed step that writes no reason shows only the exit code. Two exit codes have a meaning of bash: 127 means "command not found", and 126 means "found, but not executable".

#### The bash that Argent runs

Argent looks for bash in this order, and takes the first result:

1. The `scripts.bash` key. See [Available keys](/docs/reference/configuration#available-keys). A value that is set but wrong fails the step. Argent does not fall through to the next source
2. `bash` on the PATH of the tool-server
3. `/bin/bash`, then `/usr/bin/bash`. On Windows, the `bash.exe` of Git for Windows

macOS ships bash 3.2 at `/bin/bash`. Homebrew installs bash 5 earlier on the PATH. Associative arrays, `${var,,}` and `mapfile` need bash 4 or later. Set `scripts.bash` to pin one version. The report names the interpreter path in each failure.
...
- **Use LF line endings.** A `.sh` file with CRLF line endings fails under bash with `$'\r': command not found` and exit code 127. Argent does not read the source of the script, so Argent cannot warn you. Add `*.sh text eol=lf` to `.gitattributes`
