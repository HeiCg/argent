...
`scripts.maxTimeoutMs` caps the `timeout` of each step. `scripts.heapLimitMb` sets the heap of the Node process. The heap limit does not apply to a bash script. See [Available keys](/docs/reference/configuration#available-keys).
