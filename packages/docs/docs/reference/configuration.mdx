...
| `scripts.bash`             | project, global | project wins     | The absolute path of the bash that runs a `.sh` script step. Unset ⇒ the first bash on the PATH of the tool-server, then `/bin/bash` and `/usr/bin/bash`                        |
