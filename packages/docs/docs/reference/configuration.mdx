...
| `open-ios-device-server`  | Off     | Route iOS describe, screenshot and tap/swipe/long-press/type/key input through the open XCUITest server (`@argent/ios-device-server`) instead of the simulator-server. Experimental. Simulator and physical device |
