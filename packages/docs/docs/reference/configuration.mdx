...
| `disable-auto-screenshot` | Off     | Stop the automatic screenshot that Argent captures after each interaction tool (also stops the element tree below)            |
| `disable-auto-describe`   | Off     | Stop the accessibility element tree that Argent appends after each interaction tool, next to the automatic screenshot         |
