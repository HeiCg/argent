...
applies only on Android when the open device server is active. When `verify` is
absent, the tap and swipe paths do not change.
...
exact text, then exact content description, then a single contains match. On a
unique match Argent taps the center of the element bounds. For a swipe, Argent
starts the swipe at the center of the element bounds and keeps the end point as
given.
...
| `verify_ambiguous` | More than one element matched. The reply lists up to five candidates.                                                             |
| `verify_mismatch`  | The x and y fall outside the matched bounds ± `tolerancePx`. The reply returns the requested coordinates and the matched element. |
