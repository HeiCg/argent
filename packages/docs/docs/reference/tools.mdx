...
| `flow-finish-recording`  | Finish the recording and write the flow to disk. Returns a `requiresPrompt` when the flow declares no `requires:` block, or carries one the recorded steps contradict |
| `flow-execute`           | Run a saved flow                                                                                                                                                      |
| `flow-read-prerequisite` | Read a flow's execution prerequisite and its effective `requires:` block without running it. Refuses a composition whose folded block no target can satisfy           |
