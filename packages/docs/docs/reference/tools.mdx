...
a per-screen index and a header that names the screen version. The agent taps an
element by its index. It passes `target: { index, version }` to `gesture-tap` or to
a `gesture-sequence` `tap` step. Argent resolves the index on the device against the
same snapshot version. Argent refuses the tap with `stale_index` when the screen
moved since the `describe`, so a stale index never taps the wrong element.
