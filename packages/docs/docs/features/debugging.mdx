...
- If the app's JS runtime dies, Argent keeps the log file of the crashed session. The `debugger-log-registry` tool reports the path of that file in its note. The agent reads it before relaunching the app. Argent removes the file after one day.
