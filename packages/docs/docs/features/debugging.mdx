...
The agent reproduces a failed state. The agent restarts the app (iOS, Android, and TV — a Chromium app has to be quit and relaunched by the user), reloads the Metro bundle, grants or resets a permission, or opens a URL. Then the agent does the steps again, and reads the logs and the component tree after each step.
