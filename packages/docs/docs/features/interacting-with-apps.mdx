...
- On Chromium, the agent reads the field again, and the result says `clearVerified` when that read found the field empty. On mobile targets the result says only what the agent sent, so the agent reads the field with `describe` when it needs proof.
