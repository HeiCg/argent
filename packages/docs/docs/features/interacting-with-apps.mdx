...
The agent also clears a text field that already holds a value. The agent clears the field, then types the new value. Thus the agent replaces a value instead of adding to it. The agent removes the text before the cursor and the text after it. Thus a field with many lines also becomes empty.
...
- On mobile targets, the clear action removes up to 100 characters on each side of the cursor. A longer field keeps the remainder, and the agent clears it again. On Chromium there is no limit.
