...
A script step does work that no device step does. For example, a script calls an API that puts the backend into a known state before the flow opens the screen that shows that state. Argent runs the script on the machine that holds the project, and the step drives no device.
...
A flow also runs without an agent. The `argent flow run` command replays a flow from the terminal. The command prints the result of each step and exits with a status code. Thus a flow runs in a CI job or in a shell script. The command runs one flow, a flow file in a different location, or all flows in a directory.
