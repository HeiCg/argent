...
| Shape           | First step that is not a label or a script | Start state                                         |
| --------------- | ------------------------------------------ | --------------------------------------------------- |
...
A local script does work that no device step can do. For example, a script seeds the test data. A script is a JavaScript file or a bash file. The agent adds a script step only when you ask for one.
...
- "Record the checkout flow. Seed the cart with the scripts/seed-cart.mjs script."
- "Record the checkout flow. Seed the order with the scripts/seed-order.sh script."
