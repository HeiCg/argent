...
A local script does work that no device step can do. For example, a script seeds the test data. The agent adds a script step only when you ask for one. Argent discards what the script prints. The agent reads the verdict of the step, not the console output of the script.
