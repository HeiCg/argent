...
- The agent records a launch step for iOS and Android apps. For a Chromium or Electron app, add the launch step to the file by hand. The agent does not record flows on TV targets (Vega, Apple TV, Android TV) yet, but `argent flow run` still replays a hand-written Vega flow.
