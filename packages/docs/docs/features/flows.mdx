...
A local script does work that no device step can do. For example, a script seeds the test data. A script is an `.mjs` file or a `.sh` file. The agent adds a script step only when you ask for one.
