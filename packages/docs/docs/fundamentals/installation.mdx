...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, `argent init` prints the cause and stops before any install starts. `argent update` prints the cause and skips the install.
