...
`argent init` and `argent update` check the global directory before they run an install. When npm cannot write there, Argent prints the cause and stops before any install starts. No partial install remains.

Two ways out exist:

- Run `argent init --local`. The project gets `@swmansion/argent` as a devDependency. No global directory is needed.
