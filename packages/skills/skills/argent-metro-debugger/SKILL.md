...
2. **`reason: "no_app_connected"` → get the app to connect to Metro** — a crashed app reads as this too, so read `debugger-log-registry`'s `note` BEFORE relaunching or reconnecting - call it if you reached this from `debugger-status`, and keep what it says, since the first read spends the record. It names the log file the dead session kept, if it kept one, and a `debugger-connect` takes the record with it, reporting one only when the runtime went away rather than something tearing the session down. Then use `restart-app` on the device and retry `debugger-status` once.
