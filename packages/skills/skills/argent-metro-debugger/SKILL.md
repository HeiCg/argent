...
| Relaunch app on device            | `restart-app` (iOS / Android / Vega); on Chromium `boot-device` only starts an app, so relaunch once it has exited — `boot-device` with `electronAppPath` for Electron, or ask the user to start the browser again on the same CDP port — then re-read the id from `boot-device` / `list-devices`, since a relaunch on a new port is a new id |
