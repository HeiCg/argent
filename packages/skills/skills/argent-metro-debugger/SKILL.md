...
| Relaunch app on device            | `restart-app` (iOS / Android / Vega); on Chromium `boot-device` only starts an app, so relaunch once it has exited — `boot-device` with `electronAppPath` for Electron, or ask the user to start the browser again — then use the new `chromium-cdp-<port>` id it reports |
