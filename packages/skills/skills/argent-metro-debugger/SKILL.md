...
| Tool               | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `debugger-connect` | Connect to the JS runtime's CDP (Metro on iOS / Android / Vega; the page CDP session on Chromium). Returns port, projectRoot (empty on Chromium and on legacy Metro, e.g. Vega), deviceName, appName, `logicalDeviceId` (absent on Vega), isNewDebugger, connected, and — after the previous session for this device ended with its runtime going away — a `note` about the console history it held: the path of the log file it left, that the file it left has since been reclaimed, or that it left none. On Chromium the device id IS the CDP port, and `boot-device` draws a free one unless you pass `electronPort`, so relaunching there strands the record under the old port — read it before you relaunch, not after. Keep passing the `device_id` the connect accepted; the returned `logicalDeviceId` is informational, and you switch to it only when a multi-device Metro refuses your id (above). |
...
2. **A crash reaches several `reason`s → read the note before you relaunch** — `no_app_connected` on Metro, `cdp_unreachable` on Chromium, `stale_connection` and `device_mismatch` all follow a session that just died, and only `debugger-log-registry` carries the record, so read its `note` BEFORE relaunching or reconnecting - call it if you reached this from `debugger-status`, and keep what it says, since the first read spends the record. It names the log file the dead session kept, if it kept one, and a `debugger-connect` takes the record with it, reporting one only when the runtime went away rather than something tearing the session down. Then use `restart-app` on the device and retry `debugger-status` once.
