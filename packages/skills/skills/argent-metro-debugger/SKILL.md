...
1. **Call `debugger-log-registry`** and check `status` first. On `"connected"` it returns: `file` (log path), `totalEntries`, `byLevel`, `clusters` (top message groups with counts and source file info). On `"not_connected"` it returns `reason`, `detail`, and `guidance` with **no `file` field** — follow the `guidance`.
   Either status may also carry a **`note`**, and it is the one thing to read before acting on the rest: it says the previous session for this device was torn down (by another agent's teardown, or by the app dying) rather than the app having logged nothing, and when that session left its log file on disk the note names the path. Grep that path for the pre-crash output — it is readable even when `status` is `"not_connected"`, and a relaunched app's registry will not have those entries. Kept logs are swept a day later.
