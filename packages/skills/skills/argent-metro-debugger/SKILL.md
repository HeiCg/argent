...
2. **`reason: "no_app_connected"` → get the app to connect to Metro** — a crashed app reads as this too, so call `debugger-log-registry` and read its `note` BEFORE relaunching or reconnecting: it names the log file the dead session kept, if it kept one, and a `debugger-connect` spends the record on its way past. Then use `restart-app` on the device and retry `debugger-status` once.
