...
| Tool               | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `debugger-connect` | Connect to the JS runtime's CDP (Metro on iOS / Android / Vega; the page CDP session on Chromium). Returns port, projectRoot (empty on Chromium and on legacy Metro, e.g. Vega), deviceName, appName, `logicalDeviceId` (absent on Vega), isNewDebugger, connected, and — after the previous session for this device died with the app — a `note` about the console history it held: the path of the log file it left, or what became of those entries when it left none. When a `logicalDeviceId` comes back, use it as the `device_id` for every subsequent debugger call. |
...
2. **`reason: "no_app_connected"` → get the app to connect to Metro** — a crashed app reads as this too, so call `debugger-log-registry` and read its `note` BEFORE relaunching: it names the log file the dead session left. Then use `restart-app` on the device and retry `debugger-status` once.
