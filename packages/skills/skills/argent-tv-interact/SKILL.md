...
- `screenshot {udid, scale?}` — Apple TV via `xcrun simctl io`; Android TV / Vega host-side via `adb` / `screencap`. All three downscale to the tool-server's screenshot scale unless `scale` says otherwise.
