...
4. **Prove every screen change.** Record a destination-only identity check. During polish, follow it with `await: { idle: true }`. Stillness does not prove identity, and `idle` can pass with a warning. A `launch:` can too, on an Android expo dev build.
5. **Polish only executed behavior.** Convert recorded steps without changing their meaning. Record any missing action or structural check live. The only unrecorded insertions are a planned `snapshot:`, a navigation `await: { idle: true }`, the documented Chromium packaging `launch:`, and the two edits the [dev-client recovery](references/reliability-and-recovery.md#expo-dev-client-chooser) calls for — a `when:` block that dismisses the chooser where no `launch:` does it, and the deletion of a chooser tap an older recording kept.
