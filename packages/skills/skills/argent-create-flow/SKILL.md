...
- **Mistakes can be edited out.** Edit the `.yaml` file directly to remove or reorder steps. Mid-recording, this holds only when the tool-server shares your filesystem (the usual local case): the recorder re-reads the file before each append, so your edit is kept — provided it still parses, since the next step re-validates the whole file and fails on a botched one instead of quietly keeping it. Removing and reordering steps survives; **anything the parser does not model does not**, because the append rewrites the file from the parsed flow — comments you added while recording are dropped silently, and the call still reports success. Against a **remote client** the in-memory copy is authoritative and any write overwrites the file — the next append and `flow-finish-recording` alike, since both serialize that same copy — so edit only once the recording is finished. Editing after the recording is finished is always safe.
