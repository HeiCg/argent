...
`clear` works on iOS, Android and Chromium. **Do not reach for the `type` directive on a TV at all** — on none of the three TV families does it get as far as the `clear`, and each fails differently: Vega rejects the whole directive upfront (see _Composition and platform limits_), an Apple TV dies at the focusing tap (`gesture-tap` rejects a tvOS UDID), and an Android TV taps but cannot move D-pad focus, so the step fails the focus check and blames your selector. Type on a TV with `tool: tv-remote` (to move focus) plus `tool: keyboard` — and note the `keyboard` tool rejects `clear` on every TV family: an Apple TV's backend has no modifier-chord primitive at all and routing one through Android TV's is unverified, so neither gets a half-supported `clear`. Delete an existing value with repeated backspaces, or use the field's own clear affordance. `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`, which sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.
