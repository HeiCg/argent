...
`clear` works on iOS, Android and Chromium; it is rejected — loudly, before anything is typed — on TV targets, whose input transport cannot send the select-all chord. (On Vega the whole `type` directive is rejected, `clear` or not — see _Composition and platform limits_.) `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`. A clear-only step sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.
