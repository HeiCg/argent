...
`type` **appends**. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the step types on top of the old value and the flow fails later at an `assert`, pointing at the wrong place. `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`, which only empties the field and sends no Enter. `clear` works on iOS, Android and Chromium; it is rejected — loudly, before anything is typed — on Vega, on TV targets, and on Android versions whose `input` has no `keycombination` subcommand. The `type` step does **not** read the field back afterwards, so if you need proof the replacement took, follow it with an `assert` on the field's text.
