...
- HarmonyOS has no runner tree, so every selector directive errors with `ui-tree matching is not supported on platform "harmony"`. Point gestures, raw `tool:` steps, and `snapshot:` run. Name the device as `--device harmony-<connectKey>`: there is no `--platform harmony`, and a HarmonyOS device is never auto-resolved.
