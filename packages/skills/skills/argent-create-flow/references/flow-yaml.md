...
`clear` works on iOS, Android and Chromium; it is rejected — loudly, before anything is typed — on TV targets: an Apple TV's backend has no modifier-chord primitive at all, and routing one through Android TV's is unverified, so neither family gets a half-supported `clear`. (On Vega the whole `type` directive is rejected, `clear` or not — see _Composition and platform limits_.) `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`, which sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.
