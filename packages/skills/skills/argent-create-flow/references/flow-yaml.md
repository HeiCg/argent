...
`type` presses Enter in a second `keyboard` call unless `submit: false`. A polished focus tap plus one text-only `keyboard` call usually needs `submit: false`. A `clear`-only step (no `text`) defaults to `submit: false` instead — Enter into a field the step just emptied would fire an empty search or form.

`type` never **replaces**. The old value survives, and the new text lands wherever the focusing tap put the caret — which is not always the end. The tap goes to the middle of the field, so a value long enough to reach that point gets the new text spliced _into_ it: a field holding forty `A`s, typed with `ZZZ`, came back `AAA…ZZZ…AAA` on Chromium. A shorter value leaves the tap past the end of the text, the caret lands there, and the same step reads as an append instead — so which one you get depends on the value in the field, not on the step. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the flow fails later at an `assert`, pointing at the wrong place.

`clear` works on iOS, Android and Chromium. **Do not reach for the `type` directive on a TV at all** — on none of the three TV families does it get as far as the `clear`, and each fails differently: Vega rejects the whole directive upfront (see _Composition and platform limits_), an Apple TV dies at the focusing tap (`gesture-tap` rejects a tvOS UDID), and an Android TV taps but cannot move D-pad focus, so the step fails the focus check and blames your selector. Type on a TV with `tool: tv-remote` (to move focus) plus `tool: keyboard` — and note the `keyboard` tool rejects `clear` on every TV family: an Apple TV's backend has no modifier-chord primitive at all and routing one through Android TV's is unverified, so neither gets a half-supported `clear`. On Vega, delete an existing value with repeated `key: "backspace"` presses. On Apple TV and Android TV, `keyboard` refuses `key` as well, so the field's own clear affordance is the only way. `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`, which sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.

The step will not clear on bad evidence. It dispatches the clear on three findings, plus one window where it sees nothing at all (the residual below). The three: the tree reports focus on the element you named; or on the input inside it that the tap actually landed on — or that is the only control inside it, where the tap landed on the container's own chrome instead; or on the editing host whose whole content is what you named, which is how a `contenteditable` composer with no id and no name of its own is reachable at all. That last one also requires the host to hold **nothing but** what you named: an editor carrying a signature block, a card field or any other identified content refuses, because a clear there is a select-all over all of it.

It **fails** on anything else — a node that merely _covers_ your target (a closed shadow host, a focused WebView, a focus trap, or your own field overhanging the container's box, which default `content-box` sizing does for a few pixels), and equally a **sibling** inside the container you named that the tap missed (the currency box in a currency/amount row, one cell of an OTP row). Naming is not identity either: where two elements inside the container answer to the same id, testID or accessible name, the step cannot tell which one the tap reached and refuses — judged on the screen as it stands when focus is read, so a namesake the tap itself creates refuses too. Neither is evidence the keys will go where you meant.

One residual: when **no read in the whole focus wait** sees focus anywhere the clear does go through, because that is also what an iOS build without `firstResponder` looks like. A read that sees focus and a later one that sees none is not that case — it refuses.

Do not read that residual as "nothing was focused, so nothing can be lost". It also covers a field that IS focused and that the tree cannot report: one with no on-screen frame (positioned off screen by design, or dropped by a scroll container's clip), one an adapter prunes as invisible before focus is considered (`hidden`/`alpha` on iOS, the system-UI package on Android — the notification direct-reply field and the lock-screen bouncer), and any tree the source truncated. On Chromium an invisible focused field is kept, so the OTP-style capture input behind rendered boxes refuses there. The step reports a **pass** in every one of these, which is why the value check after a `clear` is not optional. It does not read the field back afterwards, though. If a flow must prove the new value took, follow it with an `assert` — but check [Asserting a field's value](asserting-field-values.md) first: a text field's contents reach the flow tree unevenly (never on iOS, never for a password, and on Chromium an `<input>` only as the accessible name), so the obvious assert on the field's text hard-fails on a perfectly good clear. And a **clear-only** step has no empty-state assert at all — `equals: ""` is rejected at parse time and `matches: '^$'` never matches — so assert the old value's absence, `assert: { hidden: "the old value" }`. Where the contents are invisible, assert the consequence instead.
...
- Vega uses `tool: tv-remote` and raw `tool: keyboard`. The touch directives (`tap`, `long-press`, `type`, `scroll-to`, `pinch`, `rotate`) are unsupported. That keyboard call's `clear` is rejected there too — Vega cannot send the select-all chord, so empty a field with the app's own affordance or repeated `key: "backspace"` presses. Gate focus and navigation results with `await`.
