...
- **The `output` global** starts as an empty object, and `flow-add-script` returns the document the script leaves in it. No flow step reads it yet: `parseFlow` refuses a file that writes `{{output:...}}` into one, so the whole flow fails to load rather than printing the text literally. It scans the fields a later release resolves it in - an echo message, a typed or expected text, a selector's `text`, `id` or `role`, and each string in `tool.args`. A `matches` pattern is not among them, because `{{` in a regular expression is a quantifier the author meant.
