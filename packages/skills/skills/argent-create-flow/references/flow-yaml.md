...
`type` **appends**. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the step types on top of the old value and the flow fails later at an `assert`, pointing at the wrong place. `text` may be omitted for a clear-only step: `type: { into: search, clear: true }`. `clear` works on iOS, Android (API 30+) and Chromium; it is rejected on Vega, on TV targets, and on older Android. A `clear` that does not empty the field fails the step immediately rather than appending.
