...
The step will not clear on bad evidence: it dispatches the clear only when the tree reports focus on the element you named, or on the input inside it that the tap actually landed on. It **fails** on anything else — a node that merely _covers_ your target (a closed shadow host, a focused WebView, a focus trap), and equally a **sibling** inside the container you named that the tap missed (the currency box in a currency/amount row, one cell of an OTP row). Neither is evidence the keys will go where you meant. One residual: when the tree reports focus on **nothing at all** the clear does go through, because that is also what an iOS build without `firstResponder` looks like, and what a field the app blurred — or scrolled off screen — looks like. A clear with nothing focused loses no data; a clear on the wrong field does. It does not read the field back afterwards, though. If a flow must prove the new value took, follow it with an `assert` — but check [Asserting a field's value](asserting-field-values.md) first: a text field's contents reach the flow tree unevenly (never on iOS, never for a password, and on Chromium an `<input>` only as the accessible name), so the obvious assert on the field's text hard-fails on a perfectly good clear. And a **clear-only** step has no empty-state assert at all — `equals: ""` is rejected at parse time and `matches: '^$'` never matches — so assert the old value's absence, `assert: { hidden: "the old value" }`. Where the contents are invisible, assert the consequence instead.
