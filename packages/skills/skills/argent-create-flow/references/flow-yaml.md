...
- Vega, Apple TV and Android TV use `tool: tv-remote` and raw `tool: keyboard`, and `clear: true` is rejected on all three. On Vega a `keyboard` step carries `text` or `key`; on the two TV targets `key` is rejected too, so it carries `text` only. An Android TV serial is indistinguishable from a phone's, so the "iOS, Android" rows above do not settle it — check `runtimeKind` in `list-devices`. The touch directives (`tap`, `long-press`, `swipe`, `type`, `scroll-to`, `pinch`, `rotate`) are unsupported on all three. Gate focus and navigation results with `await`.
