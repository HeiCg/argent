...
- **Apple TV and Android TV are not flow targets.** Their ids classify as `ios` and `android` — an Android TV serial is indistinguishable from a phone's, so check `runtimeKind` in `list-devices` — and the runner's touch-directive rejection is gated on Vega alone. It will therefore ACCEPT a `tap:` or `type:` on one and dispatch it at a focus-driven UI, where it does nothing useful. Nothing in the engine stops you; do not author them. `keyboard` still refuses `key` and `clear` on both. Drive those two with the tools directly, per the argent-tv-interact skill.
