...
1. Boot the app with `boot-device` and `electronAppPath`. If arguments are necessary for the app, add `electronArgs`.
2. Take one `screenshot`. Write down its pixel dimensions. The app sets its window dimensions, and no boot argument changes them. Those dimensions key the snapshot baselines, so CI must reproduce them. If the app sizes the window from host or session state, say so in the report.
...
The path is relative to `.argent/flows/`. Write the boot arguments accurately. If the boot had no arguments, do not add `args`. If the file has an `executionPrerequisite`, remove it. This is the only launch that you can add by hand.
