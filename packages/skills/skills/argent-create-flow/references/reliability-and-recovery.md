...
| Chromium | No reachable CDP session         | Still up with no window — the failure names page targets, none at all or only devtools:// ones: ask for one back, since a relaunch recovers nothing there. Otherwise ask the user to quit it, then relaunch once it has exited — `boot-device` with `electronAppPath` for Electron, or ask the user to start the browser again on the same CDP port with `--remote-debugging-port` — then re-read the `chromium-cdp-<port>` id from `boot-device` / `list-devices`, since a relaunch on a new port is a new id. `list-devices` cannot confirm the exit |
