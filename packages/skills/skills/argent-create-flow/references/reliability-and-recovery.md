...
| Environment error  | The reason says the check could not run        | Repair the environment and rerun; it is no verdict about the app. A failed `launch:` is `errored` too and **is** a verdict — treat it as a hard failure, unless it names the expo dev-client chooser's bundler (no live server on `metroPort`, or still showing after 60s); its tap-failed and unreadable-screen reasons are environment like any other |
