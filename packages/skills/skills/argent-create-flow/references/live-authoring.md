...
6. Inspect `toolResult`, `message`, and `recorded` after each call. A call that errors records nothing, and a call that returns normally while reporting an unmet condition **does** append the step; `await-ui-element` is the case that turns up in practice (see [Live waits and checks](#live-waits-and-checks)). One returning call records nothing: an Android tap that opens a server row on the expo dev-client chooser straight after the recorded `launch:`, which the launch performs itself at replay. Its `recorded` line opens with `(not recorded)` and `message` says why. Read both rather than assuming a step was added. Only `flow-start-recording` and `flow-finish-recording` return the whole YAML as `flowFile`. A step call returns `recorded` — one summary line for the step it appended — plus a running `stepCount`. Read `recorded`: the recorder does not always store the tool call you made, and that line is where a rewrite shows up. To see the whole file mid-recording, read it at `savedTo`. A `savedTo` that comes back `null` means the write failed on your side. The step is still in the recording, so continue: the next step rewrites the whole file, and `flow-finish-recording` returns `flowFile` regardless.
...
`flow-add-step` cannot receive a flow selector directly. Discover the element first, then record `gesture-tap` at its frame center; the live coordinates are transport for the gesture, not a final locator. The recorder reads the pre-tap tree and derives the selector in a fixed order — `id`, then `text`, then `role` — giving four outcomes. Read the `recorded` line after every tap, because only two of them warn. It names the derived form — a selector map, the kept point, or nothing at all:
...
4. **`(not recorded)`, and no step at all.** Android only: the tap opened a server row on the expo dev-client chooser straight after the recorded `launch:`, which opens that row itself at replay. Nothing to repair — the tap ran on the device and the flow is correct without it.
...
On Android, add `metroPort` when the app is an expo dev build and its Metro is not on port 8081 — `--metro-port` on `argent flow run`. A `launch:` step then opens that server when it lands on the dev-client chooser, which a dev build shows on every cold start. See [the recovery](reliability-and-recovery.md#expo-dev-client-chooser).
...
Manual rescue invalidates the pass. An `errored` step was never evaluated: an `idle` wait whose tree source could not be read, a step that threw, an unresolvable `run:` target, or a `launch:` that did not start the app. Read the reason — most name the environment, but a failed `launch:` is a verdict about the app. One class of launch reason is the exception: the expo dev-client chooser recovery, which is about the bundler rather than the app. Two of its reasons are bundler verdicts — the chooser listed no live server on the run's `metroPort` (`--metro-port` from the CLI), or was still showing 60s later because the bundler at that address never served this app — so repair the bundler or the port and rerun. Its other two name different subsystems: a tap that failed (`the tap that opens <url> failed: …`) points at the device, and is usually cleared by a rerun alone; a screen that could not be read for 60s afterwards says the tree source went down mid-bundle, so check the device connection, not Metro. Unconfirmed focus is not in this class at all: the replay focus poll has no failure return, so a `type:` step whose focus was never confirmed is scored a **pass**, and only the value check after typing catches it.
...
One uninterrupted full pass completes a normal flow. `argent-qa-flows` requires two consecutive passes of unchanged YAML. For CI, use `argent flow run <name> [--platform ...] [--metro-port ...]`; it exits non-zero on failure. An Android expo dev build needs `--metro-port` whenever its Metro is not on 8081.
