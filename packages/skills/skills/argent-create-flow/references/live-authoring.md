...
A warning never means the step was rejected. A raw `tool:` step replays against the tree it just passed, so treat the warning as a polish-time instruction — `flow-finish-recording` repeats each verdict in `summary` as an indented `warning:` line under the step it judges, which is where you act on it. A verdict is dropped there when a mid-recording hand edit moved the step out from under it, which is one more reason to keep edits until after the finish; `message` then counts what `summary` actually carries. Replay every conversion regardless. Keep the raw tool only when its `pollIntervalMs` or `bundleId` is required.
