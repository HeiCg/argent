...
Three return fields are not self-evident. `reason` carries the executor's note on a pass, which is where a clamped `timeout` shows up. `durationMs` is absent when the path was refused before anything ran. `outputJson` is the returned document as JSON text, and is present only on a pass; over 64 KiB it is cut, and `outputTruncated` says so.
