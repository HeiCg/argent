...
4. Treat `flow-start-recording` as destructive. It always truncates the named YAML's steps, including a finished or committed flow. An existing `requires:` block is the one thing carried across, and only when the flow file sits on the tool server's host and parses, or was deleted while a live take of it was still recording — against a remote tool server, or over a file that does not parse, nothing carries over and the message says so. Remotely that note can only warn: the server never reads that file, so whether it held a block at all is unknowable there - do not relay the note as a loss that happened. `restarted` reports only a displaced live take.
