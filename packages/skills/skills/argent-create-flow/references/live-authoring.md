...
Other returns are success-shaped and record nothing after the call already ran. A composed `flow-execute` that answers with a prerequisite notice, fails, or is cancelled writes no `run:` step, and its `message` names what happened. `run-sequence` is refused outright: the recorder would write one raw batch step, so a single nested failure discards the whole batch. Record the actions as individual `flow-add-step` calls instead. Whenever a step is refused after something ran, treat the device as possibly moved off your recorded prefix's end state — check with `describe` or `screenshot` and restore it by hand. Do not relaunch the app: that lands on the start screen, not where your prefix ended.
