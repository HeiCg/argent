...
- **ffmpeg is only needed where simulator-server cannot encode**: it records and muxes the video itself on macOS, so nothing extra has to be installed there. Its Linux and Windows builds cannot, and the video is encoded host-side instead — there `screen-recording-start` fails up front with an install hint if `ffmpeg` is missing (`brew install ffmpeg`), resolving it from `PATH` plus the usual Homebrew prefixes.
