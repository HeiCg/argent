...
Use `run:` for a separately recorded reset or seed flow, and `script:` for setup or cleanup that no recorded flow can do. Record the `script:` step live with `flow-add-script`, at the point in the walkthrough where it belongs, rather than typing it in afterward. There is no other fixture mechanism. Ask before cleanup that creates or deletes meaningful user data outside the request.
