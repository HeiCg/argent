...
Some Android emulators cannot stream a full-resolution frame and reject `scale: 1.0` with a `wrong data size` error. Drop the `scale` there to capture at the default instead.
