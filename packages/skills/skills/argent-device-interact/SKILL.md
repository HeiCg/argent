...
| --------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Target app discovery              | `describe`                | Accessibility element tree for the current device screen (iOS AX-service, Android uiautomator or the open-source on-device server, or Chromium DOM walker) with normalized frame coordinates. Works on any app, system dialogs, and Home screen — no app restart or `bundleId` required |
...
- `selector`: `{ text?, identifier?, role?, containsDescendant?, index? }` — every provided field must match. `text` matches the element's label or value and `role` its element role (e.g. `AXButton`, `button`, `TextView`, `StaticText`), both as case-insensitive substrings; `identifier` matches its accessibility id / resource-id / testID **exactly** (case-insensitive), also accepting the unqualified Android resource-id name (`submit` matches `com.example.app:id/submit`). The synthetic `ROOT` container `describe` prints is never matched, so a `role` like `AXGroup`/`html` won't trivially "match the screen".
- **Precise field matching.** Any of `text` / `identifier` / `role` also accepts an object `{ equals?, contains?, regex?, caseInsensitive? }` instead of a plain string. Its constraints are AND-ed and **case-sensitive by default** (set `caseInsensitive: true` to fold case) — reach for it when the plain string is too loose: `{ text: { equals: "Save" } }` matches "Save" but not "Save Changes"; `{ text: { regex: "^\\d{6}$" } }` matches a 6-digit code. Plain strings keep their historical case-insensitive behavior.
- **Narrowing among duplicates.** `containsDescendant: <selector>` keeps only elements whose frame contains a distinct element matching the nested selector — e.g. `{ role: "AXGroup", containsDescendant: { text: "Delete" } }` finds the card that holds a Delete button. `index: <n>` picks the Nth match (0-based, in reading/pre-order) instead of the best-ranked one, for when several elements are genuinely interchangeable and you want a specific position.
- **On a miss, read the note.** When the selector matches nothing, the timeout `note` lists the closest elements in the last tree (by attribute similarity), rendered like `describe` lines — use them to spot a typo, a wrong case, or the id you actually wanted.
