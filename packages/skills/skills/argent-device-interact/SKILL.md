...
| Restart an app    | `restart-app`       | Terminate and relaunch by bundle ID. **Not on Chromium** — relaunch the app itself (`boot-device` with `electronAppPath` for Electron, or start the browser again on its CDP port) |
