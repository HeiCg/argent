...
| Restart an app    | `restart-app`       | Terminate and relaunch by bundle ID. **Not on Chromium** — relaunch the app itself (`boot-device` with `electronAppPath` for Electron, or start the browser again on its CDP port), then take the new `chromium-cdp-<port>` udid from it: a relaunch on a new port is a new device |
