...
| Restart an app    | `restart-app`       | Terminate and relaunch by bundle ID. **Not on Chromium**, where `boot-device` only starts an app and never stops one — relaunch once it has exited (`boot-device` with `electronAppPath` for Electron, or ask the user to start a browser again), then take the new `chromium-cdp-<port>` udid from it: a relaunch on a new port is a new device |
