...
| Restart an app    | `restart-app`       | Terminate and relaunch by bundle ID. **Not on Chromium**, where `boot-device` only starts an app and never stops one — ask the user to quit it, then relaunch once it has exited (`boot-device` with `electronAppPath` for Electron, or ask the user to start a browser again on the same CDP port), then re-read the `chromium-cdp-<port>` udid from `boot-device` / `list-devices`: a relaunch on a new port is a new device |
