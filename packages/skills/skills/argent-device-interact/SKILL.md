...
**Opt-in.** The tool is hidden unless the feature flag is enabled: `argent enable run-script`. It executes model-written code locally in the tool-server process (a `node:vm` sandbox — an API boundary, not a security boundary), so it is off by default.

**Language.** The `script` is **plain JavaScript** (an async function body), _not_ TypeScript — no type annotations. The only globals are `ui` (the device facade below) and `console`; there is no `require`, `import`, `process`, `fs`, or network. `await` every `ui.*` call. Use `console.log(...)` to surface values — captured (tail-capped) in the result's `logs`.
