...
- Where that length cannot be read — a **password** field (uiautomator reports those empty), or a dump the device refused — the fallback sends a fixed 168 backspaces instead. That run is neither exact nor refusable, so a value longer than it keeps its head while the call still reports success.
