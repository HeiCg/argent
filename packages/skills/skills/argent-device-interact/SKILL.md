...
- `hidden` treats a selector that matches **nothing** as already-hidden, so a typo'd selector returns an instant (false) success. Double-check the selector for `hidden` waits — the result `note` flags when the selector never matched any element. (On iOS the tool will **not** report `hidden` success off an empty tree it cannot vouch for — the accessibility backend being down, or a native view hierarchy that could not be read at all; the `note` then carries the reason rather than a bare "no element matched".)
