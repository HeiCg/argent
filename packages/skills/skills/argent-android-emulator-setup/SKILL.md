...
- `reinstall-app` on Android always installs with `-g` so first-launch runtime permissions are pre-granted. `uninstall-app` removes an app (data + permissions) for teardown — it is idempotent, reporting `uninstalled: false` when the app was already absent rather than failing.
- `set-location` fakes the emulator's GPS (`latitude`, `longitude`) for maps / geofencing / region tests — emulators only (a physical Android device has no host-side override); it sets the coordinate, not the permission, so pair it with `settings-permissions` for the location grant.
- `enable-third-party-install` grants an installed app the `REQUEST_INSTALL_PACKAGES` app-op so a sideload / in-app-installer flow isn't blocked by the "install unknown apps" prompt.
