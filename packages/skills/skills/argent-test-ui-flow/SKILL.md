...
3. screenshot { scale: 1.0, includeImageInContext: false } → save baseline path when visual or mixed evidence needs diffing (drop `scale` if the emulator rejects a full-res frame)
...
3. screenshot { scale: 1.0, includeImageInContext: false } → save baseline path (drop `scale` if the emulator rejects a full-res frame).
