...
export const FLAG_REGISTRY: readonly FlagDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Under `open-device-server`, select the on-device touch-injection strategy for tap/swipe/gesture per RPC (phase 3n/3n.1): `uia-sync` (blocking final UP), `uia-async` (async final UP, drain folded into the next read, on the UiAutomation channel), or `input-manager` (reflective `InputManager.injectInputEvent` with `INJECT_INPUT_EVENT_MODE_ASYNC`; falls back to `uia-async` and reports `strategy:\"unavailable\"` when the hidden API is blocked). Phase 3n.1: the DEFAULT is now `input-manager` (fastest arm on-device, at-or-below the proprietary driver on every gesture verb); the `default` (alias `uia`) sentinel selects the pre-3n.1 Kotlin DEFAULT path (a tap's async UP, a swipe/gesture's blocking UP). The active value is carried by the `ARGENT_OPEN_INJECT_STRATEGY` env var (the boolean flag store cannot hold a value) so the bench can flip strategy per block in one run. No effect unless `open-device-server` is also on.",
  },
  {
    name: "screen-graph",
    description:
