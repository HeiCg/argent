...
i.e. the divergence is not _demonstrated_ significant at n=16 — it demonstrates that the
...
- **Does `input-manager` under-scroll vs proprietary on this metric?** Cannot be answered
  (instrument unresolved). Descriptively — NOT a graded finding — input-manager did _not_
...
sentence this run may add after adversarial review: _"a rebuilt, uncensored optical
...
graded."_ Next options for a future ticket (not this one): much larger N per cell, or a
