...
- **F1 idle-free rotation/package reads.** New `util/DisplayReader.kt` reads real
  ...
  the truth and the rule: _never call a `UiDevice` getter that triggers `waitForIdle`
  on a hot path_.
...
RPC (F2) and the payload trim (F4), **serialization is now 12 ms** — it is _not_ the
...
_immediate_ describe already contain the destination screen's content?
...
default sits on the _immediate_ side of the policy axis — same family as ON
...
ticket removed _is_ removed (`waitedMs` 0). What remains is the open path's nested
...
- **OFF `tap+describe` is bimodal** — the proprietary `waitForIdleMs:500` is a
  _bounded_ wait, landing on the fast/stale or the waited side run-to-run — so the
...
`62 → 61` ms p50; OFF 53. Target _ON ≤ OFF + 2_ (≤ 55) → **missed by ~8 ms**. The
...
_before_ injecting its synchronous drain event. The **describe** path is safe:
...
`90 → 63` ms p50; OFF 101 (OFF-1) / 78 (OFF-2). Target _ON ≤ OFF + 3_ → **met**
...
The scrcpy control-channel tap **lands**. Every failure this phase was an _effect
oracle_ judging the tap too early or with too coarse a signal, not a dropped touch.
