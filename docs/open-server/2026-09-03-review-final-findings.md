...
1. **The describe speedup magnitude is not reproducible.** Run 6, same
   code, measured ON describe _equal to_ OFF. The doc quotes run 5 and
   omits run 6. (F1.)
...
- **HIGH (F5)** — **The untimed locate is NOT backend-independent.** The
  ...
  not less**. The asymmetry biases _against_ ON on the tap rows, so it does
  ...
  _fingerprint_ is. Fix: report `locateVia` in the results doc, not only in
...
- **The 59/60 is a REAL scrcpy injection drop, not an oracle miss.**
  ...
  (60/60) and both OFF blocks (40/40) _in the same runs_; and the same 59/60
...
- **MEDIUM (F7)** — **The miss is undiagnosable from the artifacts.**
  ...
  implemented for _thrown_ errors only (`:591-593`, `:458-461`), which is
...
- **BLOCKING (F2)** — **The "measurement noise" explanation is refuted by
  ...
  150/0.3 = 1.296 was a uia _under-scroll_: uia 0.358 against off 0.457,
...
- **MEDIUM (F9)** — **`informative` does not exclude the observed floor.**
  ...
  `ratio = 1.000` because _both_ arms are pinned at that floor, and that
...
re-verified each line against the tree. Which have been _observed_ to fire:
...
- **effect / landing** (`merge-blocks.js:114`) — **FIRES.** Run 6's merge
  threw on the then-strict `== 0` form; confirmed by the _absence_ of any
...
- **MEDIUM (F16)** — the `paste` floor is 110 ms, an order of magnitude
  ...
  that the paste _row itself_ carries a 110 ms floor under a roughly 174 ms
...
1. **Phase 3h's stated tap mechanism**, "the scrcpy tap lands because the
   ...
   MOVE in every block, and `merge-blocks.js:53-58` now _fails_ any block
...
2. **"Open describe is faster than proprietary, 36/53 against 52/56"** as a
   ...
   OFF 52. Survives as: _ON is never slower than OFF and is 13-19 ms faster
   in 2 of 3 runs._
