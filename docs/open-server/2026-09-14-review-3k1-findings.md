...
| **3K-H2** (cross-run "resolved") | **FIXED**, one residual | Status is OPEN in both docs, the cross-run sentence is the one I asked for. Residual: "the under-scroll *did* reproduce (unlike run 34800933407)" contradicts the branch's own rewritten 3k doc, which says the 400/0.3 deficit there was real at p = 0.001 (3K1-M5). |
...
1. **3K1-H1 — the offered mechanism ("OS VelocityTracker on an 8-frame scrcpy gesture vs
   ...
   and `scrcpy-inject-timeline.ts:108-162` is a line-for-line port of it — so *both* ON
...
2. **3K1-H2 — "the host delivers on time with no stretch to remove" is published next to
   ...
   (legacy) against uia's 17 ms — i.e. the device *did* see a stretched scrcpy tail,
...
3. **3K1-H3 — the verb table is a regression diff against 34806342684 only; read
   ...
   *at parity*, which is the row now in the scoreboard. The harness's own scoreboard even
...
3. **3K1-M3 — `INCONCLUSIVE` exits 0: the gate can silently no-op.** `merge-fling.js`
   ...
   than failing the step. No test covers it (`gates.test.js` covers a *missing file*, not
...
5. **3K1-M5 — "the scrcpy under-scroll vs proprietary *did* reproduce (unlike run
...
2. **3K1-L2 — `ARGENT_SCRCPY_PACING` is read per gesture, not once per process.**
   ...
   test) — but it means the wire path of a *product* process can be changed by a stray
...
2. **The store-shape / O5 routing / O2-token values change**: settings store 10/9 (max
   out-degree 8) not 11/10/9; O5 60/60 routed, 0 diverged, 0 fallbacks; O2 tokens 54 not
   68. Recompute, do not copy.
3. **The B1 caveat and the token run-to-run spread stay sourced to D.4.1** (they are
   statements *about* run-to-run behaviour and reference 34788497583 / 34794414764).
...
Mandatory footnote on the latency block: *"ON-scrcpy ran the shipped default pacing
(`legacy`); run 34800933407's ON-scrcpy verbs ran `drift`, which was then the default."*
...
Optional supporting line (allowed, label as measurement-only): *"floor rate, samples at
the 0.175 metric floor out of 12: 150 ms/0.3 — off 1, uia 3, scrcpy drift 4, scrcpy legacy
6; 400 ms/0.5 — off 0, uia 2, drift 2, legacy 5."*
