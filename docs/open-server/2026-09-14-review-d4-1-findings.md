...
2740 — B1 waits marginally _less_ than every open config, not the most (D41-H1).
...
- **D41-H1 · The published "proof of symmetry" is the wrong statistic, and its
  ...
  only the _explicit_ settle. The open arms' post-action wait is split in two: the
  ...
  O2 2639, O3 2628, O4 2642, O5 2740. So B1 does _not_ pay the largest wait; the
...
- **D41-H2 · The headline states only half the conditionality: B1's 100/100 is as
  ...
  parses it", not "not a rendering property": B1's rendering _does_ differ from
